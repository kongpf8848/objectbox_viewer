...
void main() {
  ...
  // Print all entries (including small ones)
  for (var pgno = 0; pgno < numPages; pgno++) {
    ...
    for (var i = 0; i < uniquePtrs.length; i++) {
      ...
      print(
        'Entry off=$entryStart len=$entryLen schema=$isSchema byte15=${bytes[absEntry + 15]}',
      );
      print('  Key: $keyHex');
      print('  Value (len=$valLen):');
      if (valLen <= 64) {
        final valHex = bytes
            .sublist(valStart, valStart + valLen)
            .map((b) => b.toRadixString(16).padLeft(2, '0'))
            .join(' ');
        print('    $valHex');
        // Try interpret as integers
        if (valLen >= 4)
          print('    uint32[0]=${bd.getUint32(valStart, Endian.little)}');
        if (valLen >= 8)
          print('    uint64[0]=${bd.getUint64(valStart, Endian.little)}');
        if (valLen >= 8)
          print('    int64[0]=${bd.getInt64(valStart, Endian.little)}');
      } else {
        // Parse as FlatBuffer data
