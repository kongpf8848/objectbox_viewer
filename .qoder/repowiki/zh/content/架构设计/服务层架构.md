...
- **新增** 增强的 FlatBuffer vtable 字段计数比较机制，显著改进多实体类型在同一 LMDB B-tree 中的区分能力
...
**更新** 本次更新反映了服务层架构的重大增强，包括 freed page 检测、LMDB 复制写入语义处理，以及基于 FlatBuffer vtable 字段计数比较的 schema 匹配机制，显著提升了多实体类型在同一 LMDB B-tree 中的区分能力。
...
- 服务层：lib/services/objectbox_service.dart 封装 LMDB 文件解析、实体发现与数据读取，包含 freed page 检测功能和增强的 schema 匹配机制。
...
- [lib/services/objectbox_service.dart:1-1165](file://lib/services/objectbox_service.dart#L1-L1165)
...
## 核心组件
- ObjectBoxService：服务层核心，负责打开数据库、获取文件信息、解析 FlatBuffers 并发现实体与属性、读取实体数据，**新增 freed page 检测功能**和**增强的 schema 匹配机制**。
...
- 读取实体数据：按页扫描 LMDB 页面，解析 FlatBuffers 表，提取字段值，**处理 LMDB 复制写入语义**，**使用 vtable 字段计数进行 schema 匹配**，去重保留最新版本记录，最终输出有序数据行。

**更新** 架构现在包含了 freed page 检测、LMDB 复制写入语义处理，以及基于 FlatBuffer vtable 字段计数比较的增强 schema 匹配机制。
...
- 职责分离
  ...
  - 数据读取：按实体维度扫描页面，解析 FlatBuffers 字段，**处理 LMDB 复制写入语义**，**使用 vtable 字段计数进行 schema 匹配**，去重并排序返回结果。
- 关键方法
  - openDatabase：校验路径与文件，读取字节并交由解析器发现模型。
  - getDbFileInfo：遍历目录文件，返回文件名到大小的映射。
  - **更新** readEntityData：读取实体数据，按页扫描并解析 FlatBuffers，**过滤 freed pages**，**使用 _entryMatchesSchema 进行 vtable 字段计数比较**，保留最新版本记录。
- 错误处理
  - 对于缺失目录或文件的情况抛出异常；上层 BLoC 捕获后转换为错误状态并反馈 UI。

**更新** ObjectBoxService 现在包含了 freed page 检测功能和基于 vtable 字段计数的增强 schema 匹配机制，显著提升了数据读取的准确性和可靠性。
...
**更新** 新增了 freed page 诊断工具和 FlatBuffer 分析工具的使用指导。
...
**更新** 新增了 freed page 过滤和 vtable 字段计数预过滤对性能的积极影响。
...
**更新** 本次更新显著增强了服务层的功能，包括 freed page 检测、LMDB 复制写入语义处理，以及基于 FlatBuffer vtable 字段计数比较的增强 schema 匹配机制，大幅提升了数据读取的准确性和可靠性。这些增强功能特别适用于多实体类型共享同一 LMDB B-tree 的复杂场景，通过 vtable 字段计数这一高效的预过滤机制，显著减少了不必要的解析成本。未来可在依赖注入、性能优化与缓存策略方面进一步增强，以支撑更大规模的数据浏览场景。
...
- **新增** FlatBuffer 分析：debug_schema_parse.dart 和 trace_schema.dart 提供了详细的 FlatBuffer 结构分析功能，包括 vtable 字段计数计算和实体属性解析，有助于深入理解 schema 匹配机制的工作原理。

**更新** 新增了 freed page 诊断工具和 FlatBuffer 分析工具的使用说明。