...
- [tool/trace_schema.dart](file://tool/trace_schema.dart)
</cite>
...
- **新增** 增强的属性ID解析能力，引入新的 _ObxParser 类和更精确的字段映射机制
- **新增** 基于属性ID的字段映射机制，支持 fieldIndex = propertyId - 1 的精确映射
...
**更新** 本次更新反映了服务层架构的重大增强，包括 freed page 检测、LMDB 复制写入语义处理，以及基于属性ID解析的精确字段映射机制，显著提升了多实体类型在同一 LMDB B-tree 中的区分能力和数据读取的准确性。
...
- 服务层：lib/services/objectbox_service.dart 封装 LMDB 文件解析、实体发现与数据读取，包含 freed page 检测功能和增强的属性ID解析机制。
...
- 工具与测试：tool/* 包含调试与测试脚本，辅助验证服务层行为，包括 freed page 分析工具和 FlatBuffer 解析工具。
...
DEBUG_SCHEMA["tool/debug_schema_parse.dart"]
TRACE_SCHEMA["tool/trace_schema.dart"]
...
- [tool/debug_schema_parse.dart:1-251](file://tool/debug_schema_parse.dart#L1-L251)
- [tool/trace_schema.dart:1-223](file://tool/trace_schema.dart#L1-L223)
...
## 核心组件
- ObjectBoxService：服务层核心，负责打开数据库、获取文件信息、解析 FlatBuffers 并发现实体与属性、读取实体数据，**新增 freed page 检测功能**和**增强的属性ID解析机制**。
...
- **新增** FlatBuffer 解析工具：debug_schema_parse.dart 和 trace_schema.dart 用于分析 FlatBuffer 结构和 vtable 字段计数。
...
- 读取实体数据：按页扫描 LMDB 页面，解析 FlatBuffers 表，提取字段值，**处理 LMDB 复制写入语义**，**使用属性ID进行精确字段映射**，去重保留最新版本记录，最终输出有序数据行。

**更新** 架构现在包含了 freed page 检测、LMDB 复制写入语义处理，以及基于属性ID解析的精确字段映射机制，显著提升了数据读取的准确性和可靠性。
...
participant SCHEMA_MATCH as "Schema 匹配器"
participant FIELD_MAP as "属性ID映射器"
...
PARSER->>SCHEMA_MATCH : "解析 schema 实体"
SCHEMA_MATCH-->>PARSER : "实体信息"
PARSER->>FIELD_MAP : "建立属性ID映射"
FIELD_MAP-->>PARSER : "精确字段映射"
...
PARSER->>SCHEMA_MATCH : "vtable 字段计数比较"
PARSER->>FIELD_MAP : "属性ID字段映射"
FIELD_MAP-->>PARSER : "精确映射结果"
SCHEMA_MATCH-->>PARSER : "匹配结果"
...
- [lib/services/objectbox_service.dart:975-1038](file://lib/services/objectbox_service.dart#L975-L1038)
...
- 职责分离
  ...
  - 数据读取：按实体维度扫描页面，解析 FlatBuffers 字段，**处理 LMDB 复制写入语义**，**使用属性ID进行精确字段映射**，去重并排序返回结果。
- 关键方法
  - openDatabase：校验路径与文件，读取字节并交由解析器发现模型。
  - getDbFileInfo：遍历目录文件，返回文件名到大小的映射。
  - **更新** readEntityData：读取实体数据，按页扫描并解析 FlatBuffers，**过滤 freed pages**，**使用 _entryMatchesSchema 进行 vtable 字段计数比较**，**应用属性ID映射机制**，保留最新版本记录。
- 错误处理
  - 对于缺失目录或文件的情况抛出异常；上层 BLoC 捕获后转换为错误状态并反馈 UI。

**更新** ObjectBoxService 现在包含了 freed page 检测功能和基于属性ID解析的精确字段映射机制，显著提升了数据读取的准确性和可靠性。
...
+_entryMatchesSchema(entry, expectedFieldCount) bool
+_parsePropertyTable(tableStart, propIndex) _ParsedProperty
+_parseDataEntry(entry, entity) EntityRow?
+_inferPropertyType(value) PropertyType
}
...
- [lib/services/objectbox_service.dart:928-1038](file://lib/services/objectbox_service.dart#L928-L1038)
...
- **新增** 属性ID解析：PropertyInfo 中的 propertyId 字段用于精确映射 FlatBuffer 字段索引，实现 fieldIndex = propertyId - 1 的映射机制。

```mermaid
flowchart TD
Start(["开始"]) --> CheckSchema["检查是否存在已知 schema"]
CheckSchema --> |有| ParseSchema["解析 schema 实体与属性<br/>包含属性ID解析"]
CheckSchema --> |无| StringScan["字符串扫描候选实体名称"]
StringScan --> VTableScan["VTable 扫描推断实体"]
ParseSchema --> BuildModel["构建模型<br/>建立属性ID映射"]
...
- [lib/models/objectbox_model.dart:189-192](file://lib/models/objectbox_model.dart#L189-L192)
...
- **新增** FlatBuffer 分析：使用 tool/debug_schema_parse.dart 和 tool/trace_schema.dart 分析 FlatBuffer 结构和 vtable 字段计数，辅助理解 schema 匹配机制。
- **新增** 属性ID映射优化：基于属性ID的字段映射机制支持更精确的数据解析，当属性ID可用时，fieldIndex = propertyId - 1 的映射规则提供比传统顺序映射更高的准确性。

**更新** 新增了 freed page 诊断工具、FlatBuffer 分析工具和属性ID映射机制的使用指导。
...
- **新增** 工具依赖：debug_freedb.dart、debug_schema_parse.dart、trace_schema.dart 作为独立的诊断和分析工具，不依赖主应用框架。
...
DEBUG_SCHEMA["debug_schema_parse.dart"] --> ANALYSIS["分析工具"]
TRACE_SCHEMA["trace_schema.dart"] --> ANALYSIS
```
...
- **新增** vtable 字段计数预过滤：通过 _entryMatchesSchema 快速区分不同实体类型的 FlatBuffer 记录，减少不必要的完整解析成本。
- **新增** 属性ID映射优化：基于属性ID的精确字段映射机制减少字段解析错误，提升数据准确性。

**更新** 新增了 freed page 过滤、vtable 字段计数预过滤和属性ID映射优化对性能的积极影响。
...
- 常见错误
  ...
  - **新增** FlatBuffer 结构分析：使用 tool/debug_schema_parse.dart 和 tool/trace_schema.dart 分析 vtable 字段计数，辅助理解 schema 匹配机制。
  - **新增** 属性ID映射问题：当属性ID解析失败时，系统会回退到顺序映射机制，可通过调试工具检查属性ID提取过程。
- 排查步骤
  ...
  - **新增** 使用 tool/debug_schema_parse.dart 和 tool/trace_schema.dart 分析 FlatBuffer 结构，验证 vtable 字段计数和 schema 匹配机制。
  - **新增** 检查属性ID解析：验证 _parsePropertyTable 方法中的属性ID提取逻辑，确认 IdUid 格式的正确解析。
  - 在 UI 中查看错误视图提供的错误消息，必要时重新选择数据库目录。

**更新** 新增了 freed page 诊断工具、FlatBuffer 分析工具和属性ID映射问题的使用指导。
...
**更新** 本次更新显著增强了服务层的功能，包括 freed page 检测、LMDB 复制写入语义处理，以及基于属性ID解析的精确字段映射机制，大幅提升了数据读取的准确性和可靠性。这些增强功能特别适用于多实体类型共享同一 LMDB B-tree 的复杂场景，通过属性ID映射机制实现 fieldIndex = propertyId - 1 的精确映射，显著减少了字段解析错误。基于 vtable 字段计数的预过滤机制和 freed page 检测进一步提升了系统的鲁棒性和性能。未来可在依赖注入、性能优化与缓存策略方面进一步增强，以支撑更大规模的数据浏览场景。
...
- **新增** FlatBuffer 分析：debug_schema_parse.dart 和 trace_schema.dart 提供了详细的 FlatBuffer 结构分析功能，包括 vtable 字段计数计算和实体属性解析，有助于深入理解 schema 匹配机制。
- **新增** 属性ID映射机制：PropertyInfo 中的 propertyId 字段和 _parsePropertyTable 方法实现了精确的属性ID解析，支持 IdUid 格式的正确提取和字段映射。