...
## 更新摘要
**所做更改**
- 新增了对 ObjectBox JSON 模式文件中 "id:uid" 格式的专门处理机制
- 更新了数据模型解析流程，确保正确提取 ID 部分用于内部处理
- 增强了数据完整性验证，防止 UID 部分干扰内部标识符处理
- 新增了 PropertyInfo.propertyId 字段，用于属性ID映射功能
- 更新了属性解析逻辑，支持从 JSON schema 中提取精确的属性ID
- 增强了 FlatBuffer 数据解析中的属性ID映射能力
...
### 数据模型处理改进
**更新** 本节新增了对 ObjectBox JSON 模式文件中 "id:uid" 格式的专门处理机制

ObjectBox 在 JSON 模式文件中使用 "id:uid" 格式来表示实体、属性和索引的唯一标识符，其中：
- id：实际的内部标识符
- uid：ObjectBox 内部使用的唯一标识符

为确保数据完整性，系统现在包含专门的解析函数来正确处理这种格式：

#### ID 解析机制
- `_parseIdFromIdUid()`：提取 "id:uid" 格式中的 id 部分
- `_parseIdIntFromIdUid()`：将提取的 id 部分转换为整数

#### 应用范围
这些解析函数被广泛应用于所有 JSON 解析场景：
- ObjectBoxModel.fromJson()：解析模型版本和最后实体ID
- EntityInfo.fromJson()：解析实体ID和最后属性ID
- PropertyInfo.fromJson()：解析属性ID和索引ID
- IndexInfo.fromJson()：解析索引ID和实体ID
- RelationInfo.fromJson()：解析关系ID和实体ID
...
**章节来源**
- [lib/models/objectbox_model.dart:1-15](file://lib/models/objectbox_model.dart#L1-L15)
- [lib/models/objectbox_model.dart:40-61](file://lib/models/objectbox_model.dart#L40-L61)
- [lib/models/objectbox_model.dart:102-118](file://lib/models/objectbox_model.dart#L102-L118)
- [lib/models/objectbox_model.dart:198-207](file://lib/models/objectbox_model.dart#L198-L207)
- [lib/models/objectbox_model.dart:267-276](file://lib/models/objectbox_model.dart#L267-L276)
- [lib/models/objectbox_model.dart:292-299](file://lib/models/objectbox_model.dart#L292-L299)

### 属性ID映射功能增强
**新增** 本节介绍新增的 PropertyInfo.propertyId 字段及其应用场景

PropertyInfo 类现在包含一个重要的新字段 propertyId，用于精确映射 FlatBuffer 字段索引：

#### 字段定义与用途
- **字段名称**：propertyId
- **数据类型**：int
- **默认值**：0
- **核心作用**：映射 FlatBuffer 字段索引，计算公式为 `fieldIndex = propertyId - 1`
- **回退机制**：当 propertyId 为 0 时，使用顺序索引

#### 解析逻辑
在 PropertyInfo.fromJson() 中，propertyId 通过以下方式设置：
- 从 JSON 的 'id' 字段提取 ID 值
- 使用 `_parseIdFromIdUid()` 函数处理 "id:uid" 格式
- 将提取的字符串转换为整数
- 如果转换失败，使用 0 作为回退值

#### FlatBuffer 集成
服务层在解析 FlatBuffer 数据时利用 propertyId 进行精确映射：
- 计算字段索引：`fieldIndex = prop.propertyId - 1`
- 当 propertyId 为 0 时，回退到顺序索引：`fieldIndex = propIndex + 1`
- 支持最大属性ID：`numFields = max(propertyId) + 1`（因为 field[0] 从 0 开始）

#### 实际应用场景
- **精确字段定位**：在 FlatBuffer 数据结构中精确定位属性值
- **性能优化**：避免顺序扫描，直接访问目标字段
- **兼容性保证**：支持旧版 schema 中缺失属性ID的情况
- **数据完整性**：确保属性值与字段定义的正确对应

**章节来源**
- [lib/models/objectbox_model.dart:189-215](file://lib/models/objectbox_model.dart#L189-L215)
- [lib/services/objectbox_service.dart:758-780](file://lib/services/objectbox_service.dart#L758-L780)
- [lib/services/objectbox_service.dart:980-986](file://lib/services/objectbox_service.dart#L980-L986)

### 实体模式面板增强
**更新** 实体模式面板现在显示属性ID信息

实体模式面板（EntitySchemaPanel）已更新以显示属性ID信息：
- 在属性表格中添加了"ID"列，显示每个属性的 propertyId 值
- 支持区分不同类型的属性ID：从 JSON schema 获取的精确 ID 和运行时推断的顺序 ID
- 提供更好的调试和诊断能力

**章节来源**
- [lib/widgets/entity_schema_panel.dart:115-127](file://lib/widgets/entity_schema_panel.dart#L115-L127)
...
- 属性ID映射优化
  - **新增** 通过 propertyId 直接定位 FlatBuffer 字段，避免顺序扫描
  - 支持批量字段访问，提升大数据集处理效率
- 建议
...
- ID 解析问题
  - 如果遇到实体ID显示异常或关系解析错误，检查 objectbox-model.json 中的 "id:uid" 格式是否正确。
- **新增** 属性ID映射问题
  - 如果属性值显示为 null 或错误，检查属性的 propertyId 是否正确设置
  - 确认 FlatBuffer schema 中的属性ID与 JSON schema 一致
  - 检查是否存在属性ID冲突或重复
...
## 结论
ObjectBox Viewer 通过"发现模式"与 JSON 模式两种路径解析 ObjectBox 数据库，既保证了灵活性，又提供了稳定的可视化体验。数据模型以 FlatBuffer 为核心，结合 LMDB 页结构实现高效解析与去重。界面层通过 BLoC 管理状态，使用户能够直观地浏览实体、模式与数据。

**最新改进**：系统现已完善支持 ObjectBox JSON 模式文件中的 "id:uid" 格式，确保 ID 解析的准确性，防止 UID 部分干扰内部标识符处理。这一改进提升了数据完整性，特别是在处理复杂的实体关系和索引时更加可靠。

**新增功能**：PropertyInfo.propertyId 字段的引入显著增强了属性ID映射能力，通过精确的 FlatBuffer 字段索引映射，提升了数据解析的准确性和性能。这一功能特别适用于大型数据库和复杂 schema 场景。

建议在生产环境中配合 objectbox-model.json 使用，以获得更准确的模式信息；同时注意本地文件权限与数据备份。
...
### 属性ID映射示例
**新增** 以下示例展示了属性ID映射的工作原理：

- JSON 属性ID：`"5:123456789"`
- 解析后 propertyId：`5`
- FlatBuffer 字段索引：`fieldIndex = 5 - 1 = 4`
- 访问效果：直接定位到第 5 个字段（索引从 0 开始）

当属性ID缺失时的回退机制：
- 默认 propertyId：`0`
- 回退字段索引：`fieldIndex = 0 - 1 = -1`
- 实际使用：`fieldIndex = propIndex + 1`
